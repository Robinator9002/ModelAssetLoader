...
class ModelDownloader:
    ...
    async def download_model_file(
        ...
        logger.info(f"Starting download task {download_id} for '{hf_filename}' -> '{final_filename}'.")
        ...
        try:
            ...
            await download_tracker.fail_download(download_id, "Download cancelled by user.", cancelled=True)
            ...
            await download_tracker.fail_download(download_id, f"HTTP Error {e.response.status_code}")
            ...
            if tmp_file_path and tmp_file_path.exists():
                logger.info(f"Cleaning up temporary file {tmp_file_path} for cancelled/failed download.")
