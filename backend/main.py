...
async def search_models(
    ...
    search: Optional[str] = Query(
        None, description="Search term to query model names, descriptions, etc."
    ),
    author: Optional[str] = Query(
        None, description="Filter models by a specific author or organization."
    ),
    ...
    try:
        ...
        logger.info(
            f"Searching models via SourceManager: source='{source}', query='{search}', tags='{unique_tags}'"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="An internal error occurred while searching for models.",
        )
...
async def get_model_details(source: str, model_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="An internal error occurred while fetching model details.",
        )
...
async def configure_file_manager_paths_endpoint(
    ...
    logger.info(
        f"Configuration successful. Current base path: {response_data.configured_base_path}"
    )
...
async def download_model_file_endpoint(download_request: FileDownloadRequest):
    ...
    if not result.get("success"):
        ...
        if "not found" in error_detail.lower():
            status_code = 404
        if "gated" in error_detail.lower():
            status_code = 403
...
async def get_managed_file_preview_endpoint(
    path: str = Query(..., description="Relative path to the file to be previewed.")
):
    """
