...
import {
    ...
    type ModelDetails,
} from "../../api/api"; // Corrected import path
...
const ModelSearchPage: React.FC<ModelSearchPageProps> = ({
            ...
            [name]:
                name === "tags"
                    ? value
                          ...
                          .filter(Boolean)
                    : name === "direction"
                    ? parseInt(value, 10) // Parse string from select to number
                    : value,
        }));
    };
    ...
    const handleDirectDownloadClick = async (
        ...
        if (!isConfigurationDone) {
            alert(
                "Bitte zuerst den Basispfad in den Einstellungen konfigurieren."
            );
            return;
        }
        ...
        try {
            ...
            if (details) {
                ...
                setError(
                    `Details für ${model.id} konnten nicht geladen werden.`
                );
            }
        } catch (err: any) {
            setError(
                `Fehler beim Laden der Details für ${model.id}: ${err.message}`
            );
        } finally {
    ...
    return (
        <div className="model-search-page">
            ...
            >
                ...
                {/* --- START: Re-implemented Sorting Controls --- */}
                <div className="form-row" style={{ gap: "1rem" }}>
                    ...
                    <div className="form-group" style={{ flex: "1" }}>
                        <label htmlFor="sort-direction">Richtung</label>
                        <select
                            id="sort-direction"
                            name="direction"
                ...
                {/* --- END: Re-implemented Sorting Controls --- */}
            </form>
                ...
                {results.map((model) => (
                    <div key={model.id} className="result-item">
                        ...
                        <div className="result-item-actions">
                            ...
                            <button
                                onClick={(e) =>
                                    handleDirectDownloadClick(e, model)
                                }
                                className="button-icon"
                                disabled={
                                    !isConfigurationDone ||
